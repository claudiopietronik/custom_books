/* ============================================================================
   plot.js — piccolo motore di disegno cartesiano su SVG, condiviso da tutte
   le figure e da tutti i widget del libro.

   Uso tipico:
     const p = new Plot(svg, {xmin:-1, xmax:6, ymin:-1, ymax:5});
     p.clear(); p.grid(); p.axes();
     p.arrow(0,0, 3,4, {color:'var(--accent)', label:'v'});
     p.dot(3,4, {color:'var(--accent)'});

   Convenzioni:
   - le coordinate passate ai metodi sono SEMPRE coordinate matematiche
     (x cresce verso destra, y cresce verso l'ALTO);
   - i colori si passano come stringhe CSS, tipicamente var(--accent) ecc.,
     così le figure seguono automaticamente il tema chiaro/scuro;
   - l'aspetto è sempre isotropo (1 unità su x = 1 unità su y): senza questo,
     angoli e lunghezze sarebbero disegnati mentendo.
   ========================================================================= */
...
class Plot {
  constructor(svg, opt = {}) {
    ...
    // scala isotropa: la stessa per x e y
    this.s = Math.min(this.W / (this.xmax - this.xmin), this.H / (this.ymax - this.ymin));
    this.ox = this.W / 2 - this.s * (this.xmin + this.xmax) / 2;
    this.oy = this.H / 2 + this.s * (this.ymin + this.ymax) / 2;
  ...
  /* --- conversioni --- */
  px(x, y) { return [this.ox + this.s * x, this.oy - this.s * y]; }        // matematica → pixel
  mx(px, py) { return [(px - this.ox) / this.s, (this.oy - py) / this.s]; } // pixel → matematica
  ...
  grid(step = 1) {
    const g = svgEl("g", {});
    for (let x = Math.ceil(this.xmin / step) * step; x <= this.xmax + 1e-9; x += step) {
      const [x1, y1] = this.px(x, this.ymin), [x2, y2] = this.px(x, this.ymax);
      g.appendChild(svgEl("line", { x1, y1, x2, y2, class: "grid" }));
    }
    for (let y = Math.ceil(this.ymin / step) * step; y <= this.ymax + 1e-9; y += step) {
  ...
  arrow(x1, y1, x2, y2, o = {}) {
    ...
    if (o.label) {
      ...
      const nx = -uy / L, ny = ux / L;                 // normale, per staccare l'etichetta
      const t = svgEl("text", {
  ...
  handle(x, y, o = {}, onMove) {
    ...
    g.appendChild(svgEl("circle", { cx: a, cy: b, r: 13, fill: "transparent" })); // area di presa generosa
