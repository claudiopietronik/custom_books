...
class Plot {
  ...
  /* ==========================================================================
     MULTIVARIABILE — le aggiunte "Analisi 2"
     ======================================================================== */

  /* campo vettoriale F(x,y) = [u, v]: frecce su una griglia n×n.
     Le frecce sono scalate perché la più lunga occupi ~una maglia. */
  field(F, o = {}) {
    const n = o.n || 12;
    const dx = (this.xmax - this.xmin) / (n + 1), dy = (this.ymax - this.ymin) / (n + 1);
    const samples = [];
    let maxMag = 1e-9;
    for (let i = 1; i <= n; i++) {
      for (let j = 1; j <= n; j++) {
        const x = this.xmin + i * dx, y = this.ymin + j * dy;
        const v = F(x, y);
        if (!v || !isFinite(v[0]) || !isFinite(v[1])) continue;
        const m = Math.hypot(v[0], v[1]);
        maxMag = Math.max(maxMag, m);
        samples.push([x, y, v[0], v[1], m]);
      }
    }
    const scale = (o.scale ?? 0.9) * Math.min(dx, dy) / maxMag;
    const g = svgEl("g", {}); this.add(g);
    for (const [x, y, u, v, m] of samples) {
      const col = o.color || (o.byMag
        ? `color-mix(in srgb, var(--warm) ${Math.round(20 + 80 * m / maxMag)}%, var(--accent))`
        : "var(--accent)");
      const ar = this.arrow(x, y, x + u * scale, y + v * scale,
        { color: col, width: o.width || 1.8, opacity: o.opacity ?? 0.9 });
      g.appendChild(ar);
    }
    return g;
  }

  /* curve di livello di f(x,y) ai valori `levels` (array) — marching squares.
     opzioni: color/colors, width, labels (bool), n (risoluzione). */
  contour(f, levels, o = {}) {
    const N = o.n || 70;
    ...
    for (let i = 0; i <= N; i++) {
      val[i] = [];
      for (let j = 0; j <= N; j++) val[i][j] = f(this.xmin + i * dx, this.ymin + j * dy);
    }
    const seg = (p, q, col) => this.line(p[0], p[1], q[0], q[1], { color: col, width: o.width || 1.6, dash: o.dash });
    levels.forEach((L, li) => {
      const col = o.colors ? o.colors[li % o.colors.length] : (o.color || "var(--accent)");
      for (let i = 0; i < N; i++) {
        for (let j = 0; j < N; j++) {
          const x0 = this.xmin + i * dx, y0 = this.ymin + j * dy;
          const corners = [
            [x0, y0, val[i][j]], [x0 + dx, y0, val[i + 1][j]],
            [x0 + dx, y0 + dy, val[i + 1][j + 1]], [x0, y0 + dy, val[i][j + 1]]
          ];
          const cross = [];
          for (let e = 0; e < 4; e++) {
            ...
            if ((va - L) * (vb - L) < 0) {
              const t = (L - va) / (vb - va);
              cross.push([a[0] + t * (b[0] - a[0]), a[1] + t * (b[1] - a[1])]);
            }
          }
          if (cross.length === 2) seg(cross[0], cross[1], col);
          else if (cross.length === 4) { seg(cross[0], cross[1], col); seg(cross[2], cross[3], col); }
        }
      }
    });
    return this;
